...
namespace Cadbury.Inventor.Core.Utils
{
    public class StringUtils
    {
        public static bool ContainsURL(string input)
        {
            var regexURL = new Regex(@"(https?:\/\/)?(www\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)", RegexOptions.IgnoreCase);
            if (regexURL.IsMatch(input))
            {
                return true;
            }
            return false;
